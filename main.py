    ...
    def _test(self, test_left, test_right, last_epoch=False, save_name="", loss=None):
        ...
        if self.args.distance == 2:
            ...
            distance = torch.FloatTensor(scipy.spatial.distance.cdist(
                ...
                final_emb[test_right].cpu().data.numpy(),
                metric="cityblock"
            ))
        else:
            raise ValueError(f"Unknown distance type: {self.args.distance}")
        ...
        if not self.args.no_sinkhorn:
            ...
            try:
                ...
                log_P = sinkhorn_process(sim, temp=temperature, max_iter=60)
                final_distance = -log_P
                temp_str = str(temperature) # 用于日志显示
            except Exception as e:
                if self.rank == 0:
                    self.logger.info(f"Sinkhorn error: {e}, fallback to raw distance.")
                final_distance = distance
                temp_str = "Error"
        else:
            # if self.rank == 0:  
            #     self.logger.info("Sinkhorn Optimization is DISABLED (Ablation Study). Using raw distance.")
            final_distance = distance
            temp_str = "Disabled"
        ...
        # 在打印之前，确保我们拿到的是最新的全局最佳温度
        # if self.args.no_sinkhorn:
        #     display_temp = "Disabled"
        # else:
