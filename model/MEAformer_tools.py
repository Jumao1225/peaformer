...
#         self.name_fc = nn.Linear(768, char_dim)
...
#         # ========= [NEW] Hypergraph Encoder =========
#         if self.args.w_hypergraph:
#             # We map from entity_emb dim to hidden dim using HypergraphConv
#             # Here we assume the input to hypergraph conv is the entity embedding
#             #self.hyper_graph_model = HypergraphConv(self.input_dim, attr_dim, dropout=self.args.hyper_dropout)
#             self.hyper_graph_model = HypergraphConv(self.input_dim, self.args.attr_dim, dropout=self.args.hyper_dropout)
#         # ============================================

#         # === [修改点 1: 添加视觉图编码器] ===
#         # 我们用一个轻量级的 GCN 来处理视觉图，不干扰原来的 GAT
#         # 输入维度 input_dim (实体嵌入), 输出维度 char_dim (借用字符特征的维度位置)
#         # self.vis_gat = GAT(n_units=self.n_units, n_heads=self.n_heads, dropout=args.dropout,
#         #                    attn_dropout=args.attn_dropout,
#         #                    instance_normalization=self.args.instance_normalization, diag=True)
#         self.vis_gcn = GCN(self.input_dim, self.args.attr_dim, self.args.attr_dim, dropout=self.args.dropout)
...
#         # # ========= [NEW] Hypergraph Forward =========
#         # if self.args.w_hypergraph and hyper_adj is not None:
#         #     # Pass entity embeddings and hypergraph incidence matrix
#         #     # Note: hyper_adj should be on the same device
#         #     hyper_emb = self.hyper_graph_model(self.entity_emb(input_idx), hyper_adj)
            
#         #     # FUSION STRATEGY:
#         #     # Option A: Add to Graph Embedding (Enhance Structure)
#         #     if gph_emb is not None:
#         #         gph_emb = gph_emb + hyper_emb
#         #     else:
#         #         gph_emb = hyper_emb
#         # # ============================================
...
#         # 2. Hypergraph (属性+视觉增强)
#         if self.args.w_hypergraph and hyper_adj is not None:
#             hyper_emb = self.hyper_graph_model(self.entity_emb(input_idx), hyper_adj)
            
#             # [关键融合策略]: 
#             # 将超图特征加到 gph_emb 上，增强其语义
#             if gph_emb is not None:
#                 gph_emb = gph_emb + hyper_emb 
#             else:
#                 gph_emb = hyper_emb
...
#         else:
#             char_emb = None
#         # if vis_adj is not None and gph_emb is not None:
#         #     # 使用 GAT 提取视觉结构特征
#         #     # 注意：这里我们复用 entity_emb 作为输入
#         #     vis_gph_emb = self.vis_gat(self.entity_emb(input_idx), vis_adj)
            
#         #     # 【关键一步】：残差融合 (Residual Connection)
#         #     # 不要让它单独作为 Token，而是加到 gph_emb 上
#         #     # 系数 0.2 ~ 0.3 是一个经验值，既引入信息又不破坏原结构
#         #     gph_emb = gph_emb + 0.3 * vis_gph_emb
#         # === [修改点 2: 使用 GCN + 残差融合] ===
#         if vis_adj is not None and gph_emb is not None:
#             # 使用 GCN 提取特征 (省显存)
#             # GCN 的 forward 一般是 (features, adj)
#             vis_gph_emb = self.vis_gcn(self.entity_emb(input_idx), vis_adj)
            
#             # 残差融合 (保持这个逻辑)
#             # 0.3 是一个经验值，你可以根据情况微调
#             gph_emb = gph_emb + 0.3 * vis_gph_emb
...
#             gph_emb, 
#             name_emb, 
#             char_emb # 这里如果是 None 也没关系，fusion 会自动过滤
#         ])
...
class MultiModalEncoder(nn.Module):
    ...
    def forward(self,
                ...
                name_features=None,
                char_features=None,
                hyper_adj=None):  # <--- 参数名改为 hyper_adj 以示区分
