...
class MultiModalEncoder(nn.Module):
    def __init__(self, args,
        ...
        # 输出维度建议设小一点，或者和 attr_dim 一样
        self.topo_dim = self.args.attr_dim # 借用属性维度
        self.topo_fc = nn.Sequential(
            nn.Linear(2, self.topo_dim), # 从 2 维映射到高维
            nn.ReLU(),
    ...
    def forward(self,
                ...
                char_features=None,
                hyper_adj=None,
                topo_features=None):  # <--- 参数名改为 hyper_adj 以示区分
        ...
        if topo_features is not None:
            # [N, 2] -> [N, dim]
            topo_emb = self.topo_fc(topo_features)
        else:
            topo_emb = None
        # =========================
